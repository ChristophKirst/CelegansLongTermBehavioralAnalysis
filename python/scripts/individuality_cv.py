...
fplt.plot_image_array([dat_bin[s][order[s]] for s in strains], names = strains, vmax = 1.2, cmap = 'jet')
...
#%%
fig.savefig(os.path.join(fig_dir, 'distributions_roaming_speed_cv_sbins=%d.pdf' % sbins))
...
for nst, strain_ref in enumerate([strains[0]]):
  ...
  fig.savefig(os.path.join(fig_dir, 'cv_JS_time_signed_ref=%s_lines_sbins=%d.pdf' % (strain_ref, sbins)));
  ...
  fig.savefig(os.path.join(fig_dir, 'cv_JS_time_signed_ref=%s_matrix_sbins=%d.pdf' % (strain_ref, sbins)));
  
  np.savetxt(os.path.join(fig_dir, 'cv_JS_time_signed_ref=%s_matrix_sbins=%d.txt' % (strain_ref, sbins)), dd)
  ...
  fig.savefig(os.path.join(fig_dir, 'cv_JS_time_signed_significance_ref=%s_matrix_sbins=%d.pdf' % (strain_ref, sbins)));
...
np.savetxt(os.path.join(fig_dir, 'JS_roaming_speed_cv_sbins=%d.txt' % sbins), dd)
np.savetxt(os.path.join(fig_dir, 'JS_roaming_speed_cv_pvals_sbins=%d.txt' % sbins), pvals)
