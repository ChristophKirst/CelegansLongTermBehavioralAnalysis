...
fig_dir = '/home/ckirst/Science/Projects/CElegansBehaviour/Analysis/Roaming/Figures/2016_12_20/'
...
sts = np.array(strains)[[0,-1, 1, 2, 3, 4]]

#gs1 = gridspec.GridSpec(len(sts),1);
...
for i, s in enumerate(sts):
  ...
  plt.scatter(xdat, dat_bin[s].T.flatten() * (dbins-0.5), c = 'gray', s = 1)
  plt.xlim(-0.5, tbins-0.5)
  plt.ylim(-0.5, dbins-0.5)
  
  plt.yticks(np.linspace(0,10,6)-0.5, np.linspace(0,1,6));
  
  if i < len(sts)-1:
    #plt.setp(ax.get_xticklabels(), visible=False)
    plt.xticks(np.arange(0,51,5)-0.5, ('', '','', '','', '', '', '', '', '', ''))
  else:
    plt.xticks(np.arange(0,51,5)-0.5, ('', 'L1','', 'L2','', 'L3', '', 'L4', '', 'A', ''))

  # add average trace
  plt.plot(np.mean(dat_bin[s]*(dbins-0.5), axis = 0), 'k')
  #plt.title(s)
  plt.ylabel(s)
...
cbar_ax = fig.add_axes([0.75, 0.13, 0.22, 0.01])
fig.colorbar(im, cax=cbar_ax, orientation = 'horizontal')

fig.show()

#%%
fig.savefig(os.path.join(fig_dir, 'distributions_roaming_sbins=%d.pdf' % sbins))
...
strains_order = np.array(strains)[[-1, 1, 2, 3, 4]];
...
for nst, strain_ref in enumerate([strains[0]]):
  ...
  fig.savefig(os.path.join(fig_dir, 'JS_time_signed_ref=%s_lines_sbins=%d.pdf' % (strain_ref, sbins)));
  ...
  fig.savefig(os.path.join(fig_dir, 'JS_time_signed_ref=%s_matrix_sbins=%d.pdf' % (strain_ref, sbins)));
  ...
  fig.savefig(os.path.join(fig_dir, 'JS_time_signed_significance_ref=%s_matrix_sbins=%d.pdf' % (strain_ref, sbins)));



#%% Write data 

dd = np.zeros((len(strains_order),  ent_t_ref['tdc-1'].shape[0]));
for i, s in enumerate(strains_order):
  dd[i] = ent_t_ref[s];

from scipy.special import gammaincc;
pvals = np.zeros((len(strains_order), sJS['tdc-1'].shape[0]));
for i,s in enumerate(strains_order):
  pvals[i] = gammaincc(nu / 2.0, N[s] * np.log(2) * np.abs(ent_t_ref[s]));

np.savetxt(os.path.join(fig_dir, 'JS_roaming_sbins=%d.txt' % sbins), dd)
np.savetxt(os.path.join(fig_dir, 'JS_roaming_pvals_sbins=%d.txt' % sbins), pvals)
