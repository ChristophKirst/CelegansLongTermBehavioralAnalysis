...
for strain in strains:
  ...
  for t in range(tbins):
    for w in range(nworms):
      ...
      dat_bin[strain][w,t] = np.mean(dat[strain][w, ids][rids[w][ids]]);
...
#%%
fig.savefig(os.path.join(fig_dir, '%s_roaming_speed_distributions_sbins=%d.pdf' % (strains_label, sbins)))
...
for nst, strain_ref in enumerate([strains[0]]):
  ...
  fig.savefig(os.path.join(fig_dir, '%s_roaming_speed_JS_time_signed_ref=%s_sbins=%d.pdf' % (strains_label, strain_ref, sbins)));
  ...
  fig.savefig(os.path.join(fig_dir, '%s_roaming_speed_JS_time_signed_ref=%s_matrix_sbins=%d.pdf' % (strains_label, strain_ref, sbins)));
  
  np.savetxt(os.path.join(fig_dir, '%s_roaming_speed_JS_time_signed_ref=%s_matrix_sbins=%d.txt' % (strains_label, strain_ref, sbins)), dd)
  ...
  fig.savefig(os.path.join(fig_dir, '%s_roaming_speed_JS_time_signed_significance_ref=%s_matrix_sbins=%d.pdf' % (strains_label, strain_ref, sbins)));
...
np.savetxt(os.path.join(fig_dir, '%s_roaming_speed_JS_sbins=%d.txt' % (strains_label, sbins)), dd)
np.savetxt(os.path.join(fig_dir, '%s_roaming_speed_JS_pvals_sbins=%d.txt' % (strains_label, sbins)), pvals)
np.savetxt(os.path.join(fig_dir, '%s_roaming_speed_JS_qvals_sbins=%d.txt' % (strains_label, sbins)), qvals)
