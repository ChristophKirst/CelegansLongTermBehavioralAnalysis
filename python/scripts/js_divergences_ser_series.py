...
#%%
fig.savefig(os.path.join(fig_dir, '%s_distributions_roaming_sbins=%d.pdf' % (strains_label, sbins)))
...
strains_order = np.array(strains)[1:];
...
for nst, strain_ref in enumerate([strains[0]]):
  ...
  fig.savefig(os.path.join(fig_dir, '%s_JS_time_signed_ref=%s_lines_sbins=%d.pdf' % (strains_label, strain_ref, sbins)));
  ...
  plt.imshow(dd.T, aspect = 'auto', cmap = clmp, vmax = .75, vmin = -.75)
  plt.tight_layout();
  ...
  fig.savefig(os.path.join(fig_dir, '%s_JS_time_signed_ref=%s_matrix_sbins=%d.pdf' % (strains_label, strain_ref, sbins)));
  ...
  fig.savefig(os.path.join(fig_dir, '%s_JS_time_signed_significance_ref=%s_matrix_sbins=%d.pdf' % (strains_label, strain_ref, sbins)));
...
np.savetxt(os.path.join(fig_dir, '%s_JS_roaming_sbins=%d.txt' % (strains_label, sbins)), dd)
np.savetxt(os.path.join(fig_dir, '%s_JS_roaming_pvals_sbins=%d.txt' % (strains_label, sbins)), pvals)
np.savetxt(os.path.join(fig_dir, '%s_JS_roaming_qvals_sbins=%d.txt' % (strains_label, sbins)), qvals)
