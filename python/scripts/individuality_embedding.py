...
### t-SNE e,bedding of distributions

# joint distribution vectors 

dist_data = np.zeros((tbins * nstrains, dbins));
for ns, strain in enumerate(strains):
  dist_data[ns*tbins:(ns+1)*tbins,:] = dist_t[strain].T;
...
kernel = st.gaussian_kde(Y.T, bw_method = 0.15);

Y_xy = kernel(positions)
Y_xy = np.reshape(Y_xy, xx.shape)

plt.figure(800); plt.clf();
plt.imshow(-Y_xy.T, cmap = 'gray', aspect = 'auto', extent = (xmin,xmax, ymax, ymin));
plt.tight_layout();


# plot trajectories of a strain

ns = 0;
stg = 1;


traj = Y[ns*tbins:(ns+1)*tbins,:];
plt.plot(traj[stg*sbins:(stg+1)*sbins,0], traj[stg*sbins:(stg+1)*sbins,1], 'gray')
plt.scatter(traj[:,0], traj[:,1], c = range(tbins), cmap = plt.cm.Reds)

### 3d plot of embedding: 2d + time
...
fig = plt.figure(700); plt.clf();
cmps = [plt.cm.gray_r, plt.cm.Oranges, plt.cm.Blues, plt.cm.Reds, plt.cm.Greens, plt.cm.Purples]

for ns in range(nstrains):
  ax = plt.subplot(3,2,ns+1,projection='3d')
  # plot points
  traj = Y[ns*tbins:(ns+1)*tbins,:];
  cm = cmps[ns]; #cm.N = tbins;
  for t in range(tbins-1):
    ax.plot(traj[t:t+2,0], traj[t:t+2,1], np.array([t,t+1]), c = cm(1.0* t / tbins));
  ax.scatter(traj[:,0], traj[:,1], range(tbins), c = range(tbins), cmap = cm)
  ...
  ax.contour(xx, yy, -Y_xy,  offset=0, cmap = plt.cm.gray, extend3d = False, ncontours = 25)
  ax.set_title(strains[ns])

fig.savefig(os.path.join(fig_dir, 'distribution_embedding_genotype_tbins=%d_3D.pdf' % tbins))


### 2d plot of embedding

fig = plt.figure(701); plt.clf();
cmps = [plt.cm.gray_r, plt.cm.Oranges, plt.cm.Blues, plt.cm.Reds, plt.cm.Greens, plt.cm.Purples]

for ns in range(nstrains):
  ax = plt.subplot(3,2,ns+1)
  # plot points
  traj = Y[ns*tbins:(ns+1)*tbins,:];
  cm = cmps[ns]; #cm.N = tbins;
  for t in range(tbins-1):
    ax.plot(traj[t:t+2,0], traj[t:t+2,1], c = cm(1.0* t / tbins));
  ax.scatter(traj[:,0], traj[:,1], c = range(tbins), cmap = cm)
  ...
  # plot projection of density onto z-axis
  ax.contour(xx, yy, -Y_xy, cmap = plt.cm.gray, ncontours = 25)
  ax.set_title(strains[ns])

fig.savefig(os.path.join(fig_dir, 'distribution_embedding_genotype_tbins=%d.pdf' % tbins))




### 2d plot embedding stage wise


fig = plt.figure(702); plt.clf();
cmps = [plt.cm.gray_r, plt.cm.Oranges, plt.cm.Blues, plt.cm.Reds, plt.cm.Greens, plt.cm.Purples]
...
for h in range(2):
  for nst in range(5):
    ...
    ax.contour(xx, yy, -Y_xy, cmap = plt.cm.gray, ncontours = 25)
    if h==0:
      ax.set_title(stages[nst] + ' 1st');
    else:
      ax.set_title(stages[nst] + ' 2nd');      
  
    for ns in range(nstrains):
      # plot points
      traj = Y[(ns*tbins + nst*sbins + h*sbins2):(ns*tbins+nst*sbins + h*sbins2 + sbins2+ nodd),:];
      cm = cmps[ns]; #cm.N = tbins;
      for t in range(sbins2+nodd):
        ax.plot(traj[t:t+2,0], traj[t:t+2,1], c = cm(0.5* t / (sbins2+nodd) + 0.5));
        #ax.plot(traj[t:t+2,0], traj[t:t+2,1], c = cm(1.0* t / sbins));
        ax.scatter(traj[:,0], traj[:,1], c = range(sbins2+nodd), cmap = cm)
...
cols = ['gray', 'k', 'lightblue', 'r', 'g', 'darkblue'];
...
for nst, strain_ref in enumerate([strains[0]]):
  ...
  fig.savefig(os.path.join(fig_dir, 'BD_time_signed_ref=%s_lines_sbins=%d.pdf' % (strain_ref, sbins)));
  ...
  fig.savefig(os.path.join(fig_dir, 'BD_time_signed_ref=%s_matrix_sbins=%d.pdf' % (strain_ref, sbins)));
